<script lang="ts">
  ...
  function handleTabKeyDown(event: KeyboardEvent) {
    ...
    if (nextIndex !== currentIndex || ["Home", "End"].includes(event.key)) {
      ...
      const button = containerElement?.querySelector<HTMLButtonElement>(
...
<button
  ...
  class="w-full h-10 pointer-coarse:h-12 cursor-pointer rounded-full text-sm outline-offset-2 mb-4
