...
<dialog
  ...
  class="fixed z-50 w-64 bg-white dark:bg-neutral-800 rounded-4xl shadow-2xl p-4 border-none m-0 overflow-visible backdrop:bg-transparent"
  onkeydown={(e) => e.stopPropagation()}
  aria-label="Color Picker"
>
  <div class="flex mb-4 gap-1" role="tablist">
    {#each tabs as tab}
      <button
        role="tab"
        ...
        class="tab-button flex-1 p-2 px-3 text-xs font-medium rounded-full outline-neutral-900 dark:outline-white focus-visible:outline-2 outline-offset-2
        {activeTab === tab.id
  ...
  <div class="grid grid-cols-5 gap-2 mb-2">
    {#each Palette as color, i}
      <button
        ...
        class="palette-button aspect-square rounded-full outline-2 outline-offset-2 focus:outline-neutral-900/50 dark:focus:outline-white/50 {getSelectedColor(
          activeTab,
        ) === color.hex
  ...
  <button
    ...
    class="w-full py-2 rounded-full text-sm outline-offset-2
  ...
  <div class="flex items-center gap-2 mb-4">
    ...
    <div class="relative flex-1">
      ...
      <input
        ...
        class="w-full pl-6 pr-3 py-2 bg-neutral-100 dark:bg-neutral-900 rounded-xl font-mono text-sm focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white"
        placeholder="000000"
      />
    </div>
  </div>
  ...
  <button
    onclick={onClose}
    class="w-full py-2 rounded-full font-semibold text-sm outline-offset-2
      bg-black dark:bg-white text-white dark:text-black
